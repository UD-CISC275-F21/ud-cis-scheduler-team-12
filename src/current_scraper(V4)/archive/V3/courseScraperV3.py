    ...
    if 'PREREQ:' in prereqSearchString:            
        ...
        while(count < lengthCoursesString):
            ...
            if (len(coursesString[count]) == 7 or len(coursesString[count]) == 6):
                ...
                requiredList.update({coursesString[count]: "false"})
                ####print"requiredList: ", requiredList)
            else:
                
                ####printretrimmedPrereqString[count], " is not valid")
                orCase = coursesString[count2]
                orCase = orCase.split('|')
                
                
                
                orList.update({orCase[count3]})
                count3+=1
                count2+=1
            count +=1
        ...
        dictMaker = len(orList)
        nameNum = 1
        select = 0
        check = int(len(orList))
        
        prereqsFinal.update({"required" : requiredList})
        
        if(len(orList) > 0):
            while (dictMaker - 1 > 0):
                
                if(check == 1):
                    orDict.update({orList[select] : "false"},)
                elif(check == 2):
                    orDict.update({orList[select] : "false", orList[select+1] : "false"},)
                elif(check == 3):
                    orDict.update({orList[select] : "false", orList[select+1] : "false", orList[select+2] : "false"})
                
                field = 'optional' + str(nameNum)
                prereqsFinal.update({field : orDict})
                orDict=[]
                nameNum +=1
                dictMaker -=1
                select +=1
                                       
        #orDict.update([orList[1][0] : "false", orList[1][1] : "false"])
